...
- **링크 임베드**: ✅ 완료 — `plugins/remark-embed.mjs`. 링크만 한 줄에 두면 임베드로 변환(문장 속 링크는 텍스트 유지).
  ...
  - CodePen·StackBlitz: 링크 → 실행 가능한 코드 샌드박스 iframe
  - Vimeo 등도 같은 구조로 확장 가능
- **글쓰기·읽기 기능 확장(개발/언어 학습)**: ✅ 완료
  - 콜아웃 `> [!NOTE]`(5종) · 각주 `[^1]`(한국어 라벨) · 소제목 앵커(#) 링크
  - 정답 가리기 `||내용||`(자기주도 학습) · 읽어주기(선택 → TTS, 한/일/중/프/영 자동 추정)
  - 연재(시리즈): 같은 `series` 글을 순서대로 묶어 표시(CMS 필드 포함)
